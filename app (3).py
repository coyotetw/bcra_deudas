...
def procesar_actual(data: dict) -> tuple[str, int | None, float | None]:
    ...
    Retorna (nombre, situacion, deuda_total).
    """
    ...
    for p in periodos:
        for e in p.get("entidades", []):
            for s in e.get("situaciones", []):
                sit = s.get("situacion", 0)
                if sit > max_sit:
                    max_sit = sit
                monto = s.get("monto")
                if monto:
    ...
    situacion = 1 if not periodos else (max_sit or 1)
...
def procesar_historico(data: dict, nombre_actual: str) -> tuple[str, int | None]:
    """
    Extrae nombre (si falta) y cantidad de registros con situación > 1.
    Retorna (nombre, manchas).
    """
    if data.get("notFound") or data.get("error") or not data.get("results"):
